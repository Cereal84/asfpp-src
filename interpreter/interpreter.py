...
# Initialize the command line parser and check for argparse library
try:
...
def print_error(error_msg, error_lineno):
    """Return the Error Message and the line where it is locate"""
    error_msg = "%s - Line %d" %(error_msg, error_lineno)
    print error_msg
    raise SyntaxError
    ...
    while(1):
        if os.path.exists(filename):
            ...
            while(1):
                ...
                print "Option %s not valid" %(str(answer))
...
def p_statement_list(p):
    ...
    # Check that the list identifier is not already used
    if (str(p[2]) in global_symbol_table.keys() or 
        str(p[2]) in symbol_table.keys()):
            print_error("\tError : ID '%s' is already declared %s" 
                    %(str(p[2])), str(p.lineno(2))) 
...
# An attack is either: i) a single move/destroy invocation; ii) a conditional attack block; or iii) an unconditional attack block
def p_attack(p):
...
def p_conditional_attack(p):
    ...
    if str(p[5]) not in lists.keys():
        print_error("\tError : list '%s' is not declared %s" 
                    %(str(p[5]), str(p.lineno(5))))
...
# 2) variable declaration/definition;
# 3) packet declaration
def p_statements(p):
...
def p_statement_varinit(p):
    ...
    if str(p[2]) in reserved_name:
        print_error("""\tError : '%s' is a reserved name and can't be overwrite %s""" %(str(p[2]), str(p.lineno(2))))		
    elif str(p[2]) in symbol_table.keys():
        print_error("\tError : '%s' is already declared %s" %(str(p[2]), str(p.lineno(2))))
    else :
...
def p_init_string(p):
    ...
    p[0] = "<value>%s</value><type>STRING</type>"%(value)
...
def p_init_real(p):
    ...
    p[0] = "<value>%s</value><type>NUMBER</type>"%(str(p[1]))
...
def p_statement_move(p):
    ...
    # 'move' invocations are grouped according to their occurrence time.
    # They are also further sub-grouped according to specified coordinates
    moveArgs = "%s:%s:%s" %(str(p[7]), str(p[9]), str(p[11]))
	
    if not p[5] in listMove.keys():
        listMove[p[5]] = {}
	      
    if not moveArgs in listMove[p[5]].keys():
        listMove[p[5]][moveArgs] = "%s" %(str(p[3]))
...
def p_statement_put(p):
    ...
    args = "%s:%s:%s:%s:%s" %(str(p[3]), str(p[5]), str(p[7]), 
            str(p[9]), str(p[11]))
...
def p_error(p):
    tok = lexer.token()
    print "Syntax error - Line %s  %s" %(str(tok.lineno), str(tok))
...
def write_file(out_name):
    ...
    if (len(listDestroy) or len(listMove)) :
