	...
	*/
	...
	for(xmlpp::Node::NodeList::iterator k = details.begin(); k != details.end(); ++k) {
		...
		if(detail->get_name() == "filter") {
			...
			((ConditionalAttack *)(attack))->addFilter(get_node_value(detail));
		...
		*/
		if(detail->get_name() == "action") {
			...
			/* "Destroy" does not require any additional parameters */
			...
			/* "Move" requires the new coordinates to be specified */
			if(action_name == "Move") {
			  
				NodeLocation_type location;

				if(mobilityModule == NULL)
					opp_error("Isn't possible create a Move action. Remember that the UnconditionalAttack cant perform Move action.");

				location = mobilityModule -> getLocation();

				location.x = (atol)(tokens[0].c_str());
				location.y = (atol)(tokens[1].c_str());
				location.z = (atol)(tokens[2].c_str());

				//debug<<"\t-> x,y,z " <<location.x <<", " << location.y <<", " << location.z<<endl;
				Move *move = new Move(  mobilityModule, location);
				attack->addAction(move);
				//debug<< "\t-> Add Move Action"<<endl;	
			...
			*/
			...
			/* "Drop" requires the name of the packet to be discarded */
			if(action_name == "Drop") {
